...
{
    public partial class ClustoringSimulationPage : PhoneApplicationPage
    {
     ...
     //   MapLayer myLocationLayer = null;
        ...
        public ClustoringSimulationPage()
        {
            InitializeComponent();
        ...
        public void RemplirList()
        {
            ...
            double latitude =48.857049;
            double longitude=2.350599;
            double speed=2.0;
            var r = new Random();
            for (int i = 0; i < 500; i++)
            {
                speed = r.Next(1, 30) + Math.Round(r.NextDouble(), 6);
                latitude = r.Next(48, 50) + Math.Round(r.NextDouble(), 6);
                longitude = r.Next(2, 3) + Math.Round(r.NextDouble(), 6);
               
                myList.Add(new Data() { Latitude = latitude, Longitude = longitude, Speed = speed });
            }


            for (int i = 0; i < 500; i++)
            {
                speed = r.Next(1, 40) + Math.Round(r.NextDouble(), 6);
                latitude = r.Next(48, 50) - Math.Round(r.NextDouble(),6);
                longitude = r.Next(2, 3) - Math.Round(r.NextDouble(), 6);
